...
const YourLibrary = () => {
  ...
  const handleAddSongSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        error.response?.data?.error || "An error occurred while adding the song"
      );
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:8000/api/songs/${songToDelete.id}/delete/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleUpdateSongSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8000/api/songs/${selectedSong.id}/update/`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {songs.map((song) => (
          <div
            key={song.id}
            className="bg-gray-800 text-white rounded-lg shadow-md hover:shadow-xl transition duration-300"
          >
          ...
          <div className="bg-white p-6 rounded-lg max-w-lg w-full">
            ...
            <form onSubmit={handleAddSongSubmit} className="space-y-4">
              ...
              <textarea
                ...
                value={newSong.bio}
                onChange={(e) =>
                  setNewSong({ ...newSong, bio: e.target.value })
                }
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-md"
              />
              ...
              <input
                type="file"
                onChange={(e) =>
                  setNewSong({ ...newSong, audioFile: e.target.files[0] })
                }
                accept="audio/*"
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-md"
      ...
      <Dialog open={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-lg w-full">
            ...
            <p>
              Are you sure you want to delete the song "{songToDelete?.title}"?
            </p>
